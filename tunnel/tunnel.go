...
func NewTunnel(faddr, baddr string, clientMode bool, crypotMethod, secret string, size uint32) *Tunnel {

    a1, err := net.ResolveTCPAddr("tcp", faddr)
    if err != nil {
        log.Fatalln("resolve fronted error: ", err)
    }
...
func (t *Tunnel)transport(conn net.Conn, cipher *Cipher) {
